...
$_ENV['sd'] = "";
	...
	if($xmlConfig = simplexml_load_file($conf))
	{
		...
		$_ENV['code_notif_hdp'] = $CONFIG->CODE_NOTIF_HDP;		
		
		$_ENV['code_notif_hdr'] = $CONFIG->CODE_NOTIF_HDR;		
...
function sofie_alert_processing($typeAlert,$msg_ref,$gw_uri,$etat=0,$code_notif=0,$part_of_content=0)
{
	$gw_uri_base = $gw_uri;
	...
	if($stid->num_rows > 0) 
	{
	    ...
	    {		
			...
			echo $UpdateQuery2 = "INSERT INTO `t_notification` (DateHeureNotif,MotifNotif,IDPanne,IDNumAppel)VALUES(NOW(),$code_notif,f_getPanneIdByPanneTicket(".$row['NumPanne']."),f_getIDNumAppelAgentByIDAgent(".$row['IDAgent']."))";
...
//Create a log file in the application folder
	...
	sofie_alert_processing($typeAlert1,"ALERT_HD_PRISE_CDE",$kannel_sendsms_uri,1,$_ENV['code_notif_hdp']);
	
	sofie_alert_processing($typeAlert2,"ALERT_HD_REPARATION",$kannel_sendsms_uri,2,$_ENV['code_notif_hdr']);
