//jQuery(document).ready(function() {
	var refreshTimer = 30000; //Temps de rafraichissement des points en ms.
	...
	{	
		...
		if(idRegion !='6')
		{
			myIcon = L.icon({
			iconUrl: path,
			iconSize: [8, 12], //Taille d'icone pour la carte des regions
			iconAnchor: [9, 21],
			popupAnchor: [0, -14]
		});
		}
			...
			xhr = $.ajax({
				...
				success : function(donnee){
					$.each(donnee, function (key,val) {
						 setMarker(val.CodeOuvrage, val.Latitude, val.Longitude, val.TypeOuvrage, val.libelleStatut, url + '/' + val.icone, val.NomLocalite, val.dateStatut);
