	...
	public function getConfig($idRegion)
	{
		...
		try{			
			...
			if($idRegion != 0)
			{
				...
				$regions = DB::table('t_region')->whereRAW('IDRegion = ' . $idRegion)
									//->whereRAW('t_localite.IDRegion = ' . $idRegion)
			...
			{
				/*
				$regions = DB::table('t_region')->leftJoin('t_localite','t_region.IDRegion','=','t_localite.IDRegion')
									->whereNotNull('t_localite.IDRegion')
									->orderBy('t_region.IDRegion', 'asc')
									->lists('NomRegion', 't_region.IDRegion');	
				
						$query = DB::table('t_region')
								->whereExists(function ($query) {
								$query->select(DB::raw(1))
								->from('t_localite')
								->whereRaw('t_region.IDRegion','=','t_localite.IDRegion')								
								})
								->orderBy('t_region.IDRegion', 'asc')
								->lists('NomRegion', 't_region.IDRegion');	
				*/
