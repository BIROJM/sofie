...
{
	statComite::$localite = $_GET['localite'];
	$localite = Stat::getlocaliteName(statComite::$localite);
}
else statComite::$localite ="";
if(!empty($_GET['id'])) 
{
	statComite::$id = $_GET['id'];
	$comiteList = Stat::getComiteName(statComite::$id);
}
else statComite::$id ="";
...
$_SESSION['criteria'] = "Période : " . $periode ." - Etendu : " . $regionName . " - Localité : " . $localite . ' - Comite : ' . $comiteList;
?>
	...
	<div class="col-xs-10">
		<div class="box">
			<div class="box-header">
				<div class="box-name">
					<i class="fa"></i>
					<span>Liste des comités eau<span class="libStatGen"> - Critères : [<?php echo $_SESSION['criteria']; ?>]</span></span>
				</div>
				<div class="box-icons">
			...
			<table class="table table-bordered table-striped table-hover table-heading " id="datatable-3">
					...
					<tfoot>
							<tr>
								...
								<th class="libStatFooter"><?php echo number_format(Stat::divide(@$cumul_efficacite,count($comites)); ?> %</th>	
								<th class="libStatFooter"><?php echo number_format(Stat::divide(@$cumul_delaiMoyReparation,count($comites)); ?></th>
...
{
	var link = 'http://<?php echo $_SESSION['localIp'] . $_SESSION['url_stat']; ?>print/printListComite.php' + '<?php echo '?date_debut=' . $_GET['date_debut'] . '&date_fin=' . $_GET['date_fin'] . '&regions=' . $_GET['regions'] . '&localite=' . $_GET['localite'] . '&id=' . StatComite::$id; ?>';
