...
{	
	...
	public static function getStatutOuvrage()
	{
		...
		$sql = "select count(t_ouvrage.statutPanne) as nb, t_statutpanne.id as id from t_ouvrage, t_statutpanne
				where t_ouvrage.statutPanne = t_statutpanne.id and t_ouvrage.validated = 1 and t_ouvrage.deleted_at is null 
	...
	{
		$db = DB::loadDbConnection();
		$sql = "select count(idNotification) as nb, DATE(dateHeureNotif) as dateAppel, motifNotif, t_notification.IDPanne  from t_notification , t_panne ";
		...
		$sql .= " group by DATE(dateHeureNotif), t_notification.IDPanne, motifNotif ";
		$lines = $db->query($sql);
		$lines = $lines->fetchAll();
		return $lines;	
	}
	...
	public static function nbrePanneReparation($typePanne)
	{
		...
		if(!empty(self::$region)) 
		{
			$sql .= " and t_panne.IDOuvrage = t_ouvrage.IDOuvrage and t_ouvrage.deleted_at is null and t_ouvrage.IDRegion = " . self::$region ;
		}
		if(!empty(self::$localite))
		{		
			$sql .= " and t_ouvrage.IDLocalite = " . self::$localite ;
		...
		//$sql .= " group by t_notification.IDPanne, MotifNotif ";
		$lines = $db->query($sql)->fetchColumn();
		return $lines;
	...
	*/
	...
	public static function cmdHorsDelai($typeNotif)
	{
		$db = DB::loadDbConnection();
		$sql = "select count(t_notification.IDPanne) from t_notification, t_panne ";
		if(!empty(self::$region)) $sql .= ", t_ouvrage ";
		...
		$sql .=" and DateHeureNotif between '" . self::$periodeDebut . "' and '" . self::$periodeFin . "'";
		//$sql .= " group by t_notification.IDPanne";
		$lines = $db->query($sql)->fetchColumn();
		return $lines;
	}
