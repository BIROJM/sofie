...
{	
	...
	public static function getAgentsFormen()
	{
		...
		$sql = "select distinct(IDAgent), IDNumAppel, NomAgent, PrenomsAgent from t_agent 
				left join t_localite on t_localite.IDAgentForma = t_agent.IDAgent where t_agent.IDNumAppel is not null and t_localite.IDAgentForma is not null and t_agent.deleted_at is null ";				
	...
	public static function getSociologues()
	{
		...
		where Qualification = 4 and t_agent.IDRegion = t_region.IDRegion and t_region.IDSociologue = t_agent.IDAgent and t_agent.deleted_at is null ";				
	...
	public static function getDr()
	{
		...
		$sql = "select t_agent.IDAgent, t_agent.IDNumAppel, t_agent.NomAgent, t_agent.PrenomsAgent, t_agent.IDRegion, t_region.NomRegion 
		from t_agent, t_region 
		where Qualification = 5 and t_agent.IDRegion = t_region.IDRegion and t_agent.deleted_at is null ";				
		if(!empty(self::$region)) $sql .= " and t_agent.IDRegion = " . self::$region; 
		$sql .= " order by t_agent.NomAgent, t_agent.PrenomsAgent asc";
		$lines = $db->query($sql);
		$lines = $lines->fetchAll();
	...
	public static function nbreTransfertAbsence($idNumAppel)
	{
		...
		$sql = "select count(t_appeltelephonique.IDNumAppel) as nb
				from t_appeltelephonique 
				where t_appeltelephonique.MotifAppel = 5 
	...
	public static function nbrePannesRegion($idRegion)
	{
		$db = DB::loadDbConnection();
		$sql = "select count(t_appeltelephonique.IDNumAppel) as nb 
				from t_appeltelephonique, t_ouvrage, t_panne
				where t_appeltelephonique.MotifAppel = 1 and t_appeltelephonique.IDPanne = t_panne.IDPanne and t_panne.IDOuvrage = t_ouvrage.IDOuvrage
	...
	public static function nbreReparationsRegion($idRegion)
	{
		$db = DB::loadDbConnection();
		$sql = "select count(t_appeltelephonique.IDNumAppel) as nb 
				from t_appeltelephonique, t_ouvrage, t_panne
				where t_appeltelephonique.MotifAppel = 4 and t_appeltelephonique.IDPanne = t_panne.IDPanne and t_panne.IDOuvrage = t_ouvrage.IDOuvrage
				and  t_ouvrage.IDRegion = '" . $idRegion . "'
				and DateHeureAppel between '" . self::$periodeDebut . "' and '" . self::$periodeFin . "'";	
				//echo $sql;
		$lines = $db->query($sql)->fetchColumn();
	...
	public static function nbreReparationHorsDelaiRegion($idRegion)
	{
		$db = DB::loadDbConnection();
		$sql ="select count(t_notification.IDNumAppel) as nb
		from t_notification, t_ouvrage, t_panne 
		where t_notification.MotifNotif = 9 and t_notification.IDPanne = t_panne.IDPanne and t_panne.IDOuvrage = t_ouvrage.IDOuvrage
		and  t_ouvrage.IDRegion = '" . $idRegion . "' 
		and DateHeureNotif between '" . self::$periodeDebut . "' and '" . self::$periodeFin . "'";
	...
	public static function creationValide($userID)
	{
		...
		$sql = "select count(IDOuvrage) from t_ouvrage
			   ...
			   and t_ouvrage.deleted_at is null and validated = 1 and validated_at between '" . self::$periodeDebut . "' and '" . self::$periodeFin . "'";	
	...
	public static function creationNonValide($idRegion)
	{
		...
		and (t_ouvrage.validated = 0 or t_ouvrage.validated IS NULL) and t_ouvrage.deleted_at is null ";
	...
	public static function suiviNonValide($idRegion)
	{
		...
		where t_ouvrage.IDOuvrage = t_collecte.IDOuvrage and t_ouvrage.IDRegion = " . $idRegion ." 
		and (t_ouvrage.validated = 0 or t_ouvrage.validated IS NULL)";
		$lines = $db->query($sql)->fetchColumn();
		return $lines;
	}
	...
	public static function tauxRealisationSociologue($creationValide, $creationNonValide, $suiviValide, $suiviNonValide)
	{
		...
		if($nbreNonValide != 0)
		{
			return ($nbreValide / $nbreNonValide) * 100;
		}
		else return 0;
	}
	...
	public static function nbreComiteEau()
	{
		...
		$sql = "select count(IDComite) from t_comite 
				left join t_localite on t_localite.IDLocalite = t_comite.IDLocalite where t_comite.deleted_at is null ";				
		if(!empty(self::$region)) $sql .= " and t_localite.IDRegion = " . self::$region;
		$lines = $db->query($sql)->fetchColumn();
		return $lines;
	}
